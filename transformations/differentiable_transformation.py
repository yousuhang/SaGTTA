...
class CommonStyle(nn.Module):
    def __init__(self, n_channels=1):
        ...
        self.bias = nn.Parameter(torch.zeros(1, n_channels, 1, 1, dtype=torch.float32),
                                 requires_grad=True)  # batch x ch x h x w
        self.range = nn.Parameter(1e-3 * torch.ones(1, n_channels, 1, 1, dtype=torch.float32), requires_grad=True)

        self.uniform_dist = lambda size, device: 2 * torch.rand(size=size,
                                                                device=device) - 1  # uniform distribution in the range [-1, 1]
...
class Brightness(CommonStyle):
    ...
    def forward(self, x):
        ...
        random_brightness = self.bias + torch.abs(self.range) * self.uniform_dist((x.size()[0], self.n_channels, 1, 1),
                                                                                  x.device)  # shape is batch x ch x 1 x 1
...
class Contrast(CommonStyle):
    ...
    def forward(self, x):
        ...
        random_contrast = 1.0 + self.bias + torch.abs(self.range) * self.uniform_dist(
            (x.size()[0], self.n_channels, 1, 1), x.device)  # shape is batch x ch x 1 x 1
        x = x * random_contrast
...
class Gamma(CommonStyle):
    ...
    def forward(self, x):
        ...
        random_gamma = 1.0 + self.bias + 1e-2 * torch.abs(self.range) * self.uniform_dist(
            (x.size()[0], self.n_channels, 1, 1), x.device)  # shape is batch x ch x 1 x 1
        x = x ** (random_gamma)
...
class RandomSpatial(nn.Module):
    ...
    def generate_random_affine(self, batch_size, seed=None):
        raise NotImplementedError

    def forward(self, x, seed=None):
        random_affine = self.generate_random_affine(x.size()[0], seed=seed)
...
class RandomResizeCrop(RandomSpatial):
    ...
    def generate_random_affine(self, batch_size, seed=None):
        ...
        affine = (1 - torch.abs(self.delta_scale_x)) * self.scale_matrix_x + (
                    1 - torch.abs(self.delta_scale_y)) * self.scale_matrix_y + \
