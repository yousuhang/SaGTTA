...
def process_images(rootdir, outdir, include_seg=False, depth_interpolation=True):
    ...
    for index, img_dict in enumerate(data):
        ...
        if depth_interpolation:
            ...
            reference_physical_size = (np.array(img_dict['image'].GetSize()) - 1) * np.array(
                img_dict['image'].GetSpacing())
            ...
            # Modify the transformation to align the centers of the original and reference image instead of their
            # origins.
            centering_transform = sitk.TranslationTransform(dimension)
            img_center = np.array(
                img_dict['image'].TransformContinuousIndexToPhysicalPoint(np.array(img_dict['image'].GetSize()) / 2.0))
            ...
            if include_seg:
                aug_seg = sitk.Resample(img_dict['mask'], reference_seg, centered_transform, sitk.sitkNearestNeighbor,
                                        0.0)

        else:
            aug_image = img_dict['image']
            # aug_image = sitk.Resample(img_dict['image'], [128, 128, 128], sitk.Transform(),
            # sitk.sitkNearestNeighbor, img_dict['image'].GetOrigin(), new_spacing, img_dict['image'].GetDirection(),
            # sitk.sitkLinear, 0.0, img_dict['image'].GetPixelID())
            if include_seg:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--outdir', default = '/home/syou/SpinalCord_test' ,help='Output directory for the processed images.')
    parser.add_argument('--include_seg', default=True, action='store_true', help='Include segmentations.')
    parser.add_argument('--no_depth_interpolation', default=True,
