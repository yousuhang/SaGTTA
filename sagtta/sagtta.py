...
from utils.utils import natural_sort, ensure_dir, clip_gradient, overlay_segs, getcolorsegs

DEBUG = True
STYLE_AUGMENTORS = [Gamma.__name__, GaussianBlur.__name__, Contrast.__name__, Brightness.__name__, Identity.__name__]
SPATIAL_AUGMENTORS = [RandomResizeCrop.__name__, RandomHorizontalFlip.__name__, RandomVerticalFlip.__name__,
                      RandomRotate.__name__, RandomScaledCenterCrop.__name__]
...
class SOptTTA_GC():
    ...
    def initialize(self):
        ...
        self.target_test_dataloader = GenericVolumeDataset(self.opt.dataroot, self.opt.target_sites, phase='test')

        ##### load pre-trained style manipulator and unet-segmentor
        self.unet = UNet(self.opt.ncolor_channels, self.opt.n_classes, bilinear=self.opt.bilinear)
        self.baseline = GlobalMinimumBaseline()
        self.interpolation = LinearImageInterpolator(self.opt.inter_steps)
        self.IG = IG_simple(self.unet, self.baseline, self.interpolation)
    ...
    def load_pretrained(self):
        # unet
        latest = natural_sort(
            [f for f in os.listdir(os.path.join(self.opt.checkpoints_source_segmentor, 'saved_models')) if
             f.endswith('.pth')])[-1]
        print('Loading checkpoint: ', latest)
        weights = torch.load(os.path.join(self.opt.checkpoints_source_segmentor, 'saved_models', latest),
                             map_location='cpu')
    ...
    def smooth_loss(self, x, feats, ref_sal1, ref_sal2, cal_sal1, cal_sal2):
        loss = {}
        p = torch.softmax(x, dim=1)

        # entropy maps
        entropy = torch.sum(-p * torch.log(p + 1e-6), dim=1).mean()  # E[p log p]
        loss['entropy'] = (100 * entropy) * 1. * self.opt.alpha_0
        # match bn stats
        loss_bn = 0
        ...
        loss['saliency'] = -1 * self.cos_sim(cal_sal1, cal_sal2) * self.opt.alpha_3
        # -1*self.criterian_l1(cal_sal1,torch.zeros_like(cal_sal1))-1*self.criterian_l1(cal_sal2,torch.zeros_like(cal_sal1))#-1*(self.cos_sim(cal_sal1, ref_sal1)+self.cos_sim(cal_sal2, ref_sal2))# -1*torch.abs(self.criterian_l1(torch.abs(cal_sal1),torch.abs(cal_sal2)))*self.opt.alpha_3#
        return loss
    ...
    def optimize_augmentors(self, target_image, augmentors_list, optimizer, n_steps, sample_size):
        ...
        print("Optimizing parameters for Augmentations: {}".format(
            ', '.join([type(n).__name__ for n in augmentors_list])))
        style_augmentors = [aug for aug in augmentors_list if type(aug).__name__ in STYLE_AUGMENTORS]
        spatial_augmentors = [aug for aug in augmentors_list if type(aug).__name__ in SPATIAL_AUGMENTORS]
        if self.opt.sal1_train_path:
            sal_1_train = torch.tensor(np.load(self.opt.sal1_train_path), device=self.opt.gpu_id)
            sal_2_train = torch.tensor(np.load(self.opt.sal2_train_path), device=self.opt.gpu_id)
        else:
            sal_1_train = None
            sal_2_train = None
        if DEBUG:
        ...
        for i in tqdm.tqdm(range(n_steps)):
            ...
            # apply augmentations
            selected_aug_imgs = target_image[random.choices(slice_indices, k=sample_size)]
            aug_imgs = []
            accumulation_steps = sample_size
            cos_max_inds = torch.ones(sample_size, dtype=torch.int)
            preds = []
            loss_collect = 0
            losses_collect = {}
            if i == 0:
                if sal_1_train:
                    for j in range(selected_aug_imgs.shape[0]):
                        aug_img = selected_aug_imgs[j:j + 1]
                        sal_aug1 = self.IG.execute(aug_img, 1)
                        sal_aug2 = self.IG.execute(aug_img, 2)
                        cos_sims = torch.zeros(sal_1_train.shape[0])
                        for sal_ind in range(sal_1_train.shape[0]):
                            cos_sims[sal_ind] = self.cos_sim(torch.abs(sal_aug1),
                                                             torch.abs(sal_1_train[sal_ind:sal_ind + 1])) \
                                                + self.cos_sim(torch.abs(sal_aug2),
                                                               torch.abs(sal_2_train[sal_ind:sal_ind + 1]))
                        cos_max_inds[j] = torch.argmax(cos_sims)

            for j in range(selected_aug_imgs.shape[0]):
                aug_img = selected_aug_imgs[j:j + 1]
                if sal_1_train:
                    sal1 = sal_1_train[cos_max_inds[j]:cos_max_inds[j] + 1]
                    sal2 = sal_2_train[cos_max_inds[j]:cos_max_inds[j] + 1]
                else:
                    sal1 = None
                    sal2 = None
                for aug in style_augmentors:
                    aug_img = aug(aug_img)
                for aug in spatial_augmentors:
                    seed = np.random.choice(100000, 1)[0]
                    aug_img = aug(aug_img, seed)
                if sal1:
                    sal1 = aug(sal1, seed)
                    sal2 = aug(sal2, seed)
                aug_imgs.append(aug_img)
                pred, feats = self.unet(aug_img, feats=True)
                sal_aug1 = self.IG.execute(aug_img, 1)
                sal_aug2 = self.IG.execute(aug_img, 2)
                preds.append(pred)
                losses = self.smooth_loss(pred, feats, sal1, sal2, sal_aug1, sal_aug2)
                if j == 0:
                    for k in losses.keys():
                        losses_collect[k] = losses[k] / accumulation_steps
                else:
                    for k in losses.keys():
                        losses_collect[k] += losses[k] / accumulation_steps
                loss = sum([v / accumulation_steps for v in losses.values()])
                loss_collect += loss
                if optimizer is not None:
                    loss.backward()
                    if (j + 1) % accumulation_steps == 0:
                        clip_gradient(optimizer)
                        # loss_record = loss_collect
                        # loss_collect = 0
                        optimizer.step()
            aug_imgs = torch.cat(aug_imgs)
            preds = torch.cat(preds)
            # free up gpu
            for k in losses_collect.keys():
                losses_collect[k] = losses_collect[k].detach().cpu().item()
            losses_collect['total'] = loss_collect.detach().cpu().item()

            # moving average for visualization
            self.metric_tracker.update_metrics(losses_collect)
            ...
            if DEBUG:
                if i % 50 == 0:
                    pred_visuals.append(torch.argmax(preds, dim=1, keepdim=True))
    ...
    def test_time_optimize(self, target_image, target_image_name, batch_size=12, best_k_policies=3):
        ...
        if os.path.exists(OPT_POLICY_CHECKPOINT):
            ...
            all_augmentations = [Gamma, GaussianBlur, Contrast, Brightness, Identity, RandomResizeCrop,
                                 DummyAugmentor]  # DummyAugmentor is combination of random flip and rotate
        ...
        for sub_policy in sub_policies:
            ...
            if not DEBUG:
                ...
                pred_visuals, aug_visuals, loss_curve = self.optimize_augmentors(target_image, augmentations, optimizer,
                                                                                 n_steps, batch_size)
        ...
        for i in best_policy_indices:
            ...
            mean_pred, viz_preds, viz_augs = self.ensemble_predictions(optimized_subpolicies[i],
                                                                                         target_image)
    ...
    def visualize_losses(self, x, policy_name, image_name):
        ...
        plt.savefig(os.path.join(self.opt.checkpoints_source_free_da, 'visuals', 'loss_curves', policy_name,
                                 image_name + '.png'))
        plt.clf()
    ...
    def visualize_segmentations(self, imgs, segs, policy_name, img_name):
        ...
        tvu.save_image(overlay_grid,
                       os.path.join(self.opt.checkpoints_source_free_da, 'visuals', 'segmentations', policy_name,
                                    img_name + '.png'))
        tvu.save_image(0.5 * img_grid + 0.5,
                       os.path.join(self.opt.checkpoints_source_free_da, 'visuals', 'segmentations', policy_name,
                                    img_name + '_img_' + '.png'))
        tvu.save_image(getcolorsegs(seg_grid),
                       os.path.join(self.opt.checkpoints_source_free_da, 'visuals', 'segmentations', policy_name,
                                    img_name + '_seg_' + '.png'))
    ...
    def visualize_imgs(self, dict_imgs, img_name):
        ...
        for k, v in dict_imgs:
            ...
            tvu.save_image(img_grid, os.path.join(self.opt.checkpoints_source_free_da, 'visuals', 'images',
                                                  k + '_' + img_name + '.png'))
    ...
    def launch(self):
        ...
        for iter, (img, seg) in enumerate(self.target_test_dataloader):
            ...
            for i in range(0, img.shape[0], BATCH):
                pred, pred_probs, all_sub_policy_mean_predictions, all_sub_policy_aug_imgs = self.test_time_optimize(
                    img[i:(i + BATCH)], all_imgs[i])
                viz = [overlay_segs(img[i:(i + BATCH)].detach().cpu(), seg[i:(i + BATCH)].detach().cpu()),
                       overlay_segs(img[i:(i + BATCH)].detach().cpu(), pred.detach().cpu())]
                viz = torch.cat(viz, dim=0)
                viz = tvu.make_grid(viz, nrow=BATCH)
                tvu.save_image(viz, os.path.join(self.opt.checkpoints_source_free_da, 'visuals', 'final_predictions',
                                                 all_imgs[i] + '.png'))
